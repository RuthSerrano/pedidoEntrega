...
try:
    conn = mysql.connector.connect(
           host='NinaRu.mysql.pythonanywhere-services.com',
           user='NinaRu',
           password='mysqlroot'
      )
...
cursor.execute("DROP DATABASE IF EXISTS `NinaRu$pedidoEntrega`;")

cursor.execute("CREATE DATABASE `NinaRu$pedidoEntrega`;")

cursor.execute("USE `NinaRu$pedidoEntrega`;")
...
TABLES['Usuarios'] = ('''
      ...
      PRIMARY KEY (`usuario`)
      ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=uutf8_general_ci;''')
...
cursor.execute('select * from NinaRu$pedidoEntrega.usuarios')
print(' -------------  Usuarios:  -------------')
...
cursor.execute('select * from NinaRu$pedidoEntrega.productos')
print(' -------------  Productoss:  -------------')
